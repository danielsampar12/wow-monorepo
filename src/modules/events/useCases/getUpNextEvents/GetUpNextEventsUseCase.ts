...
interface IGetUpNextEventsUseCase {
  date: string;
  location?: {
    latitude: number;
    longitude: number;
...
export class GetUpNextEventsUseCase {
  async execute({ date, location }: IGetUpNextEventsUseCase) {
    try {
      ...
      const result: any[] = await prisma.$queryRaw`
        ...
        from
          ...
          where es.date >= ${new Date(date)} and es.date <= ${addWeeks(new Date(date), 1)}
          ) x
        group by
          x.event_schedule_uuid,
          x.event_schedule_date,
